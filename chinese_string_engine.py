...
class Chinese_string_engine(object):
    ...
    def check_idiom(self, idiom, idiom_check):
        ...
        if enum_Puzzle_Module.Module_LzPinyin == self.puzzle_module and idiom_head_lzpinyin != idiom_check_tail_lzpinyin:
            return enum_Puzzle_Unmatch.Unmatch_LzPinyin, "首尾字拼音模糊匹配失败"

        if enum_Puzzle_Module.Module_Pinyin == self.puzzle_module and idiom_head_pinyin != idiom_check_tail_pinyin:
            return enum_Puzzle_Unmatch.Unmatch_Pinyin, "首尾字拼音精确匹配失败"

        mulpinyin = pypinyin.pinyin(idiom_check[-1], heteronym=True)[0]
        if enum_Puzzle_Module.Module_Multi == self.puzzle_module and idiom_head_pinyin not in mulpinyin:
