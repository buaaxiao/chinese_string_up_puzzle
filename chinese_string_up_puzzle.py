...
class Chinese_string_up_puzzle(QMainWindow):
    ...
    def _vip_control(self, control_id, btn=None):
        ...
        continue_dict = {
            ...
            enumBarButton_Operate.Menu_ControlId_Work_BattleSingle:
            'battlesingle',
        ...
        if control_id in continue_dict.keys():
            ...
            if 1 != config_value:
                QMessageBox.information(self, 'Very Important Person', 'VIP功能',
    ...
    def _add_moduleMenuBar(self):
        ...
        self.module_action_all.triggered.connect(
            lambda: self._on_ModuleMenuBarClick(self.module_action_all,
                                                enum_Puzzle_Module.Module_All))
        ...
        self.module_action_word.triggered.connect(
            lambda: self._on_ModuleMenuBarClick(
                self.module_action_word, enum_Puzzle_Module.Module_Word))
        ...
        self.module_action_lzpinyin.triggered.connect(
            lambda: self._on_ModuleMenuBarClick(
                self.module_action_lzpinyin, enum_Puzzle_Module.Module_LzPinyin
            ))
        ...
        self.module_action_pinyin.triggered.connect(
            lambda: self._on_ModuleMenuBarClick(
                self.module_action_pinyin, enum_Puzzle_Module.Module_Pinyin))
        ...
        self.module_action_multi.triggered.connect(
            lambda: self._on_ModuleMenuBarClick(
                self.module_action_multi, enum_Puzzle_Module.Module_Multi))
    ...
    def _add_displayMenuBar(self):
        ...
        self.display_action_all.triggered.connect(
            lambda: self._on_DisplayMenuBarClick(
                self.display_action_all, enumBarButton_Display.
                Menu_ControlId_All))
        ...
        self.display_action_user.triggered.connect(
            lambda: self._on_DisplayMenuBarClick(
                self.display_action_user, enumBarButton_Display.
                Menu_ControlId_USER))
        ...
        self.display_action_ai.triggered.connect(
            lambda: self._on_DisplayMenuBarClick(
                self.display_action_ai, enumBarButton_Display.Menu_ControlId_AI
            ))
    ...
    def _add_operateMenuBar(self):
        ...
        self.operate_action_auto.triggered.connect(
            lambda: self._on_OperateMenuBarClick(
                self.operate_action_auto, enumBarButton_Operate.
                Menu_ControlId_Work_Auto))
        ...
        self.operate_action_promote.triggered.connect(
            lambda: self._on_OperateMenuBarClick(
                self.operate_action_promote, enumBarButton_Operate.
                Menu_ControlId_Work_Promote))
        ...
        self.operate_action_battlesingle.triggered.connect(
            lambda: self._on_OperateMenuBarClick(
                self.operate_action_battlesingle, enumBarButton_Operate.
                Menu_ControlId_Work_BattleSingle))
    ...
    def _init_widget(self):
        ...
        if 1 == self.cConfigHandle.get_value_int('common', 'progam_size_fix',
                                                 0):
        ...
        self.restart_button.clicked.connect(lambda: self._restart())
    ...
    def _ai_round(self):
        ...
        if 0 == len(self.ai_results):
            ...
            self._set_output(self.ai_results[0],
                             enum_Idiom_Output.ENUM_IDIOM_OUTPUT_AI,
                             self.answer_idiom_dict_promote)
        ...
        if self.operate_action_auto.isChecked():
            if 0 == len(self.answer_idiom_dict_promote):
                ...
                self.auto_timer.start(
                    self.cConfigHandle.get_value_int(
    ...
    def _user_complete(self):
        if not self.user_input_edit.text().strip():
            return
        results = self.Chinese_string_engine.get_nextIdomByKey(
            self.user_input_edit.text().strip(), self.idiom_used['used'])
        if 0 != len(results):
    ...
    def _do_auto(self):
        if 0 != len(self.answer_idiom_dict_promote):
            self.user_input_edit.setText(
                self.answer_idiom_dict_promote[0]['idiom'])
            self.user_input_button.clicked.emit()
    ...
    def _set_output(self, result, type, answer_idiom_dict_promote=[]):
        ...
        if enum_Idiom_Output.ENUM_IDIOM_OUTPUT_USER == type:
            if self.display_action_all.isChecked(
            ) or self.display_action_user.isChecked():
            ...
            if self.display_action_all.isChecked(
            ) or self.display_action_ai.isChecked():
    ...
    def _check_userInputValid(self, idiom):
        results = []
        # 成语已使用
        if idiom in self.idiom_used[
                'used'] and not self.operate_action_battlesingle.isChecked():
            QMessageBox.warning(self, '成语已使用', '你输入的成语已在本次接龙中使用, 请重新输入!',
                                QMessageBox.StandardButton.Ok)
            return False, results

        results = self.Chinese_string_engine.get_idiom_instance(idiom)
        if 0 == len(results):
            QMessageBox.warning(self, '输入错误', '系统成语库中无该记录!',
                                QMessageBox.StandardButton.Ok)
            return False, results
        ...
        if None != self.ai_answer:
            ...
            if 0 != retCheck:
                QMessageBox.warning(self, '输入错误', chkmsg,
                                    QMessageBox.StandardButton.Ok)
                return False, results
    ...
    def _congratulation(self,
                        idiom,
                        source_type=enum_Idiom_Source.ENUM_IDIOM_SOURCE_AI):
        if not self.operate_action_auto.isChecked(
        ) and source_type != enum_Idiom_Source.ENUM_IDIOM_SOURCE_AUTO:
            self._insertOutput(FINISH_TAIL)
            QMessageBox.information(self, '你赢啦',
                                    '恭喜，你的成语：\'%s\' 击败了电脑，获得胜利!' % idiom,
                                    QMessageBox.StandardButton.Ok)
