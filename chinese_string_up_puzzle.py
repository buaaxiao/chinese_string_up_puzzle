...
class Chinese_string_up_puzzle(QMainWindow):
    ...
    def _add_moduleMenuBar(self):
        ...
        self.module_action_all.triggered.connect(
            lambda: self._on_ModuleMenuBarClick(
                self.module_action_all, enum_Puzzle_Module.Module_All
            )
        )
    ...
    def _add_displayMenuBar(self):
        ...
        self.display_action_all.triggered.connect(
            lambda: self._on_DisplayMenuBarClick(
                self.display_action_all, enumBarButton_Display.Menu_ControlId_All
            )
        )
        ...
        self.display_action_user.triggered.connect(
            lambda: self._on_DisplayMenuBarClick(
                self.display_action_user, enumBarButton_Display.Menu_ControlId_USER
            )
        )
    ...
    def _add_operateMenuBar(self):
        ...
        self.operate_action_auto.triggered.connect(
            lambda: self._on_OperateMenuBarClick(
                self.operate_action_auto, enumBarButton_Operate.Menu_ControlId_Work_Auto
            )
        )
        ...
        self.operate_action_battlesingle.triggered.connect(
            lambda: self._on_OperateMenuBarClick(
                self.operate_action_battlesingle,
    ...
    def _init_widget(self):
        ...
        # 按键绑定
        self.user_input_edit.returnPressed.connect(self.user_input_button.click)
        ...
        self.grid.addWidget(
            self.user_explain_label, 2, 0, 1, 1, alignment=Qt.AlignmentFlag.AlignTop
        )
        ...
        self.grid.addWidget(
            self.ai_explain_label, 5, 0, 1, 1, alignment=Qt.AlignmentFlag.AlignTop
        )
    ...
    def _ai_round(self):
        ...
        self.ai_results, self.answer_idiom_dict_promote = (
            self.Chinese_string_engine.get_nextIdiom(
    ...
    def _set_output(self, result, type, answer_idiom_dict_promote=[]):
        ...
        if enum_Idiom_Output.ENUM_IDIOM_OUTPUT_USER == type:
            if (
                self.display_action_all.isChecked()
    ...
    def _check_userInputValid(self, idiom):
        ...
        # 成语已使用
        if (
            idiom in self.idiom_used["used"]
            and not self.operate_action_battlesingle.isChecked()
        ):
            QMessageBox.warning(
                self,
                "成语已使用",
                "你输入的成语已在本次接龙中使用, 请重新输入!",
                QMessageBox.StandardButton.Ok,
            )
    ...
    def _congratulation(
        self, idiom, source_type=enum_Idiom_Source.ENUM_IDIOM_SOURCE_AI
    ):
        if (
            not self.operate_action_auto.isChecked()
