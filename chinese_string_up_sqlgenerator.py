...
class CSqlGenerator:
    ...
    def function_all(self):
        strsql = "select * from `" + self.table_name + "` a where substr(a.idiom, 1, 1) = '" + self.idiom[-1] + "'"
    ...
    def function_multi(self):
        strsql = "select * from `" + self.table_name + "` a where a.pinyin_fst in('" + "','".join(self.mulpinyin) + "')"
        return strsql
